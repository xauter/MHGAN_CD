...
def evolution(Lab, Lab_Ref):
    ...
    return TN, TP, FN, FP, kappa, OA, auc, recall, F1
...
class Kern_AceNet(ChangeDetector):
    def __init__(self, translation_spec, **kwargs):
        ...
        self.train_metrics["feature_x"] = tf.keras.metrics.Sum(name="feature_x MSE sum")
        self.train_metrics["feature_y"] = tf.keras.metrics.Sum(name="feature_y MSE sum")
    ...
    def __call__(self, inputs, training=False):
        ...
        if training:
            ...
            retval = [x_code, y_code,x_re, y_re,x_tilde, y_tilde, struct_x, struct_y, z_discr, y_discr, di_kmeans_reverse]
    ...
    def train_step(self, x, y, clw, gt):
        ...
        with tf.GradientTape(persistent=True) as tape:
            x_code, y_code,x_re, y_re,x_tilde, y_tilde, struct_x, struct_y, z_discr, y_discr, di_kmeans = self(
                [x, y], training=True
            )
            ...
            feature_x_loss = self.loss_object(x_code, x_re)
            feature_y_loss = self.loss_object(y_code, y_re)
            tot_gen_loss = recon_x_loss + recon_y_loss + struct_x_loss + \
                           struct_y_loss + l2_gen_loss + fooling_loss + fooling_y_loss+ \
                           feature_x_loss + feature_y_loss
        ...
        self.train_metrics["feature_x"].update_state(feature_x_loss)
        self.train_metrics["feature_y"].update_state(feature_y_loss)
...
if __name__ == "__main__":
    ...
    for epoch1 in range(epoch1):
        final_kappa, final_auc= test("Italy")
        if final_kappa > 0.76 :
            print("helloworld")
            exit()
