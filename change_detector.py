...
class ChangeDetector:
    ...
    @image_to_tensorboard(static_name=None)
    def _domain_difference_img_log_ratio(
            self, original, transformed, bandwidth=tf.constant(3, dtype=tf.float32)
    ):
        """
            Compute log-ratio difference image in one domain between original image
            in that domain and the transformed image from the other domain.
            Bandwidth governs the norm difference clipping threshold
        """
        # Ensure all values are positive before taking the log
        min_value = tf.reduce_min(tf.concat([original, transformed], axis=-1))
        min_value = tf.maximum(min_value, 1e-6)
        original = original + (min_value - tf.reduce_min(original))
        transformed = transformed + (min_value - tf.reduce_min(transformed))

        # Calculate the log-ratio
        log_original = tf.math.log(original)
        log_transformed = tf.math.log(transformed)

        # Compute the difference
        d = log_transformed - log_original

        # Normalize the difference to be between 0 and 1
        d -= tf.reduce_min(d, axis=[1, 2], keepdims=True)
        d /= tf.reduce_max(d, axis=[1, 2], keepdims=True) + 1e-6

        # Clip the difference to the bandwidth
        mean_d = tf.reduce_mean(d, axis=[1, 2], keepdims=True)
        std_d = tf.math.reduce_std(d, axis=[1, 2], keepdims=True)
        threshold = mean_d + bandwidth * std_d
        d = tf.clip_by_value(d, -threshold, threshold)

        # Reduce the last dimension to get a 2D image
        d = tf.reduce_mean(d, axis=-1)

        # Ensure the result is 4D with shape (1, 宽, 高, 1)
        d = tf.expand_dims(d, axis=-1)  # Ensure the last dimension is 1
        #d = tf.expand_dims(d, axis=0)  # Ensure the first dimension is 1
    ...
    def _difference_img(self, x, y, x_hat, y_hat):
        ...
        #d_x = self._domain_difference_img_log_ratio(x, x_hat, name="x_ut_diff")
        #d_y = self._domain_difference_img_log_ratio(y, y_hat, name="y_ut_diff")
    ...
    def evaluate(self, x, y, target_change_map, filter_=None):
        ...
        if self.metrics_history["cohens kappa"][-1] >= 0.75 :#and self.metrics_history["AUC"][-1] > 0.90
            print("END!!!")
            os.system("pause")
    ...
    @image_to_tensorboard(static_name="None")
    def evolution(self,Lab, Lab_Ref):
        M, N = Lab_Ref.shape
        # fusion = confusion_map(tf.expand_dims(tf.cast(Lab_Ref, tf.bool), axis=2),
        # tf.expand_dims(tf.cast(Lab, tf.bool), axis=2))
        # img = Image.fromarray((fusion.numpy() * 255).astype("uint8"))
        # img = img.convert("RGB")
        CM_route = "D:/Louxiangyu/实验结果/confusion.bmp"
        # img.save(CM_route)

        X1 = np.int32(np.reshape(Lab_Ref, [1, M * N]))
        G1 = np.int32(np.reshape(Lab, [1, M * N]))
        [[TN, FP], [FN, TP]] = confusion_matrix(  # 真阴性(tn)、假阳性(fp)、假阴性(fn)和真阳性(tp)
            X1.flatten(), G1.flatten())
        kappa = cohen_kappa_score(X1[0], G1[0])
        ...
        # print('TN = %d,TP = %d,\nFN = %d,FP = %d,\nkappa = %f,\nOA= %f,\nauc= %f,\nrecall= %f,\nF1= %f' % (
        #    TN, TP, FN, FP, kappa, OA, auc, recall, F1))
        return TN, TP, FN, FP
...
def test(DATASET="xidian"):     # default:Texas
...
def pic(x, name, chanel='L'):
    ...
    img.save(r'./testpic/{0}.bmp'.format(name))
