...
def affinity1(x):
    ...
    Compute the affinity matrices of the patches of contained in a batch.
    It first computes the distances between the datapoints within a patch.
    Then it estimates the local density for each patch and derives the kernel width.
    Finally, applies the RBF using density-based adaptive kernel width.
        Input:
            x - float, array of [batch_size, patch_size, patch_size, num_channels],
                Batch of patches from data domain x.
        Output:
    ...
    # Reshape the input to calculate pairwise distances
    x_1 = tf.expand_dims(tf.reshape(x, [-1, h * w, c]), 2)  # Shape: [batch_size, patch_size^2, 1, num_channels]
    x_2 = tf.expand_dims(tf.reshape(x, [-1, h * w, c]), 1)  # Shape: [batch_size, 1, patch_size^2, num_channels]

    # Compute pairwise distances between pixels in the patch
    A = tf.norm(x_1 - x_2, axis=-1)  # Shape: [batch_size, patch_size^2, patch_size^2]

    # Step 1: Estimate local density for each data point
    # Density is estimated using kernel density estimation (KDE)
    density = tf.reduce_mean(tf.exp(-A ** 2), axis=2)  # KDE for each point (row-wise mean over distances)

    # Step 2: Compute adaptive kernel width based on local density
    # Kernel width is inversely proportional to density
    krnl_width = 1.0 / (density + 1e-8)  # Avoid division by zero
    krnl_width = tf.reshape(krnl_width, [-1, h * w, 1])  # Reshape for broadcasting

    # Step 3: Normalize distances using the adaptive kernel width
    A = tf.exp(-(tf.divide(A, krnl_width) ** 2))  # Apply RBF kernel using the adaptive kernel width

    return A

def affinity2(x, alpha=0.5, k=5):
    """
    Compute the affinity matrices of the patches of contained in a batch with LOS adjustment.
    It first computes the distances between the datapoints within a patch.
    Then it estimates the local density for each patch and derives the kernel width.
    Finally, applies the RBF using density-based adaptive kernel width and incorporates Local Outlier Score (LOS).

        Input:
            x - float, array of [batch_size, patch_size, patch_size, num_channels],
                Batch of patches from data domain x.
            alpha - float, weight for LOS adjustment.
            k - int, number of neighbors for kNN density estimation.
    ...
    # Compute pairwise distances between pixels in the patch
    distances = tf.norm(x_1 - x_2, axis=-1)  # Shape: [batch_size, patch_size^2, patch_size^2]

    # Step 1: Estimate local density for each data point
    # Density is estimated using kernel density estimation (KDE)
    density = tf.reduce_mean(tf.exp(-distances ** 2), axis=2)  # KDE for each point (row-wise mean over distances)
    ...
    # Step 3: Compute Local Outlier Score (LOS)
    knn_indices = tf.argsort(distances, axis=-1)[:, :, :k]  # Get indices of the k nearest neighbors
    knn_densities = tf.gather(density, knn_indices, batch_dims=1)  # Gather densities of kNN, shape: [batch_size, patch_size^2, k]
    mean_knn_density = tf.reduce_mean(knn_densities, axis=-1)  # Mean density of neighbors, shape: [batch_size, patch_size^2]
    los = density / (mean_knn_density + 1e-8)  # Compute LOS, shape: [batch_size, patch_size^2]
    los = tf.reshape(los, [-1, h * w, 1])  # Reshape for broadcasting

    # Step 4: Normalize distances using the adaptive kernel width and LOS adjustment
    A = tf.exp(-(tf.divide(distances, krnl_width) ** 2))  # Apply RBF kernel using the adaptive kernel width
    A = A * tf.exp(-alpha * tf.maximum(los, tf.transpose(los, perm=[0, 2, 1])))  # Adjust affinity matrix with LOS
