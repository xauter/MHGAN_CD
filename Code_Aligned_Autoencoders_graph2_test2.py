...
class Kern_AceNet(ChangeDetector):
    def __init__(self, translation_spec, **kwargs):
        ...
        self.train_metrics["total"] = tf.keras.metrics.Sum(name="tot_loss MSE sum")
    ...
    def train_step(self, x, y, clw, gt):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            kernels_loss = self.loss_object(Kern, zx_t_zy)
            recon_x_loss = self.loss_object(x, x_tilde)
            recon_y_loss = self.loss_object(y, y_tilde)
            hat_x_loss = self.loss_object(x_hat, graph_x)
            hat_y_loss = self.loss_object(y_hat, graph_y)

            tot_gen_loss = recon_x_loss + recon_y_loss + \
                           l2_gen_loss + fooling_loss + fooling_y_loss + \
                           kernels_loss+\
                           hat_x_loss + hat_y_loss
        ...
        #self.train_metrics["total"].update_state(tot_loss)
