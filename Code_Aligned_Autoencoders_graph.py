...
from network import ImageTranslationNetwork,  Graph_Attention_new,Graph_Attention_Chs,Graph_Attention_Chs1,Graph_Attention_Chs2
...
class Kern_AceNet(ChangeDetector):
    def __init__(self, translation_spec, **kwargs):
        ...
        #self._graph_net = Graph_Attention_Chs(in_channel=3, out_channel=3)
        ...
        self.train_metrics["hat_x"]= tf.keras.metrics.Sum(name="hat_x MSE sum")
        self.train_metrics["hat_y"]= tf.keras.metrics.Sum(name="hat_y MSE sum")
    ...
    #@image_to_tensorboard()
    #def graph_net(self, inputs, training=False):
    #    return self.graph_net(inputs, training)
    ...
    def __call__(self, inputs, training=False):
        ...
        if training:
            ...
            retval = [x_code,y_code,x_hat,y_hat,x_tilde, y_tilde, struct_x, struct_y, z_discr, y_discr, di_kmeans_reverse]

        else:
            #graph_net_x = Graph_Attention_Chs2(3, 3, out_channel=11)
            #graph_net_y = Graph_Attention_Chs2(3, 3, out_channel=3)
            ...
            #z_x=graph_net_x(x_code, y_code)
            #z_y=graph_net_y(y_code, x_code)
            #pic(z_x, name="z_x", chanel="RGB")
            #pic(z_y, name="z_y", chanel="RGB")
            difference_img = self._difference_img(x_tilde, y_tilde, x_hat, y_hat)
    ...
    def train_step(self, x, y, clw, gt):
        ...
        with tf.GradientTape(persistent=True) as tape:
            x_code,y_code,x_hat,y_hat,x_tilde, y_tilde, struct_x, struct_y, z_discr, y_discr, di_kmeans = self(
                [x, y], training=True
            )
            #graph_net_x = Graph_Attention_Chs2(3,3, out_channel=3)#多头
            #graph_net_y = Graph_Attention_Chs2(3,3, out_channel=1)
            ...
            #hat_x_loss = self.loss_object(x_hat, graph_net_x(x_code, y_code))
            #hat_y_loss = self.loss_object(y_hat, graph_net_y(y_code, x_code))

            hat_x_loss = self.loss_object(x_tilde, graph_net_x(x_code, y_code))
            hat_y_loss = self.loss_object(y_tilde, graph_net_y(y_code, x_code))

            tot_gen_loss = recon_x_loss + recon_y_loss + struct_x_loss + \
                           struct_y_loss + l2_gen_loss + fooling_loss + \
                           fooling_y_loss + hat_x_loss + hat_y_loss
        ...
        self.train_metrics["hat_x"].update_state(hat_x_loss)
        self.train_metrics["hat_y"].update_state(hat_y_loss)
...
def test(DATASET="xidian", CONFIG=None):
    ...
    for epochs in CONFIG["list_epochs"]:
        ...
        tr_gen, dtypes, shapes = datasets._training_data_generator2(
    ...
    cd.final_evaluate(EVALUATE, **CONFIG)
    #cd.final_evaluate1(EVALUATE, **CONFIG)#California
...
if __name__ == "__main__":
    ...
    for epoch1 in range(epoch1):
        ...
        #if final_kappa > 0.82 and final_auc > 0.9:#xidian
        #if final_kappa > 0.7 :#Italy
            print("helloworld")
            exit()
